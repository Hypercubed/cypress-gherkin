...
type Visitor<U, P> = (node: U, index: number, parent: P, next: NextFunction) => any
...
export class Walker {
  ...
  private walkFeature(feature: Feature, index: number, parent: GherkinDocument) {
    const next = () => feature.children.map((child, i) => {
      if (child.rule) {
  ...
  private walkRule(rule: Rule, index: number, parent: Feature) {
    const next = () => (rule.children || []).map((child, i) => {
      if (child.scenario) {
  ...
  private walkScenario(scenario: Scenario, index: number, parent: Feature | Rule) {
    if (scenario.examples && scenario.examples.length) {
      const _next = () => (scenario.examples || []).map((examples, i) => {
    ...
    const next = () => (scenario.steps || []).map((step, i) => this.walkStep(step, i, scenario));
  ...
  private walkExamples(examples: Examples, index: number, parent: Scenario) {
    ...
    const next = () => (examples.tableBody || []).map((tableRow) => {
      ...
      const _next = () => (_steps || []).map((step, i) => this.walkStep(step, i, parent));
  ...
  private walkBackground(background: Background, index: number, parent: Feature | Rule) {
    const next = () => (background.steps || []).map((step, i, _steps) => this.walkStep(step, i, background));
